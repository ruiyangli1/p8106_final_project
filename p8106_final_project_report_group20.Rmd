...
gbm.grid <- expand.grid(n.trees = c(1000,2000,3000), # B
                        interaction.depth = 1:8, # d
                        shrinkage = c(0.0008,0.001,0.0012), # lambda
                        n.minobsinnode = c(13,14)) # min # of obs in terminal nodes
...
Figure 4 displays the distribution of the training errors (in black) of each of the models that we considered in this project. We can see that both random forest (rf) model and boosting (gmb) model performed well compared to the other models. Both models have similar performance using the training data; the random forest model has a slightly larger mean RMSE but a smaller median RMSE than the boosting model. As there is extreme value in RMSE, median RMSE can be a more appropriate criterion as it is insensitive to extreme values whereas mean is. Hence, we decided to use the random forest model, which has a smaller median RMSE, as our final model. 
...
var_distn
```
\center __Figure 1__ Variable Distribution (Training Dataset) \center
...
scat_plt 
```
\center __Figure 2A__ Scatter Plot of Continuous Variable vs. Crime Rate (Training Dataset) \center
...
box_plt
```
\center __Figure 2B__ Boxplot of Categorical Variable vs. Crime Rate (Training Dataset) \center
...
corrplot(cor(data0[trRows,]), method = "circle", type = "full", tl.cex = 0.53)
```
\center __Figure 3__ Correlation Plot (Training Dataset) \center
...
model_compare
```
\center __Figure 4__ Predictive Performance of All Models  \center
...
```
\center __Figure 5__ The Variable Importance Plot (VIP), Partial Dependence Plots (PDP), Individual Conditional Expectation (ICE) Curves from Random Forest Model (Final Model) \center
...
Since Linear Model has a closed form solution, there is no parameter we have to tune.

## 2. Ridge Model

We tuned 100 lambda values in between $e^2$ to $e^{-3}$. From the graphical output below, the lambda value with the smallest cross-validation RMSE lies between 0 and 1.
...
```
\center __Figure 1__ Tuning Lamda of Ridge Model \center

\pagebreak

## 3. Lasso Model
 
We tuned 100 lambda values in between $e$ to $e^{-5}$. From the graphical output below, the best lambda value with the smallest cross-validation RMSE lies between -1 to 0.

```{r}
# --- lasso
lasso.tun.plt = plot(lasso.fit, xTrans = log); lasso.tun.plt
```
\center __Figure 2__ Tuning Lamda of Lasso Model \center
...
```
\center __Figure 3__ Tuning alpha and Lamda of Elastic Net Model \center

\pagebreak

## 5. PCR Model

We tuned 12 components and the following graph shows the optimal components number is 8.

```{r}
# --- pcr
pcr.tun.plt = ggplot(pcr.fit, highlight = TRUE); pcr.tun.plt
```
\center __Figure 4__ Tuning Optimal Components of PCR Model \center

\pagebreak

## 6. PLS Model

We tuned 12 components and the following graph shows the optimal components number is 10.

```{r}
# --- pls
pls.tun.plt = ggplot(pls.fit, highlight = TRUE); pls.tun.plt
```
\center __Figure 5__ Tuning Optimal Components of PLS Model \center
...
We tuned the degree from 1 to 3 and the value of nprune from 2 to 22. The best model with the smallest cross-validation RMSE is with nprune = 9 and degree = 2.

```{r}
# --- MARS model
mars.tun.plt = ggplot(mars.fit); mars.tun.plt
```
\center __Figure 6__ Tuning MARS Model \center

\pagebreak

## 9. Regression Tree Model

We tuned 50 Complexity Parameters from $e^{-6}$ to $e^{-3}$. The model with the smallest cross-validation RMSE is with cp = 0.01376379.

```{r}
# --- regression tree
rpart.tun.plt = ggplot(rpart.fit, highlight = TRUE); rpart.tun.plt
```
\center __Figure 7__ Tuning Complexity Parameter of Regression Tree Model \center
...
# ctree.fit$bestTune
```
\center __Figure 8__ Tuning Mincriterion of Conditional Inference Tree Model \center

\pagebreak

## 11. Random Forest Model

We tuned number of selected variable at each split from 1 to 13, and tuned the minimum node size from 
1 to 6. The model with the smallest cross-validation RMSE is with number of selected variable at each split equal to 1 and minimum node size equal to 4.
...
# rf.fit$bestTune
```
\center __Figure 9__ Tuning Random Forest Model \center

\pagebreak

## 12. Boosting Model

We tuned number of trees with value equal to 1000, 2000 and 3000, tuned interaction depth with value in between 1 to 8, tuned shrinkage value equal to 0.0008, 0.001, and 0.0012, and tuned minimum number of observations in terminal nodes equal to 13 and 14. 

The model with the smallest cross-validation RMSE is with n.trees = 2000, interaction.depth = 8, shrinkage = 0.001, and n.minobsinnode = 14.
...
# gbm.fit$bestTune
```
\center __Figure 10__ Tuning Boosting Model \center
